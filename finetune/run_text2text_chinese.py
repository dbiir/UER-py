...
class CText2text(torch.nn.Module):
    def __init__(self, args, pinyin_vocab_size=0, bushou_vocab_size=0, pinyin_bushou_vocab_ids=[]):
        super(CText2text, self).__init__()
        self.embedding = str2embedding[args.embedding](args, len(args.tokenizer.vocab), pinyin_vocab_size, bushou_vocab_size, pinyin_bushou_vocab_ids)
...
def load_pinyin_bushou_vocab(args):
    word_vocab_size = len(args.tokenizer.vocab)
    pinyin_vocab_size = len([line for line in open(args.pinyin_vocab, encoding='utf8')])
    bushou_vocab_size = len([line for line in open(args.bushou_vocab, encoding='utf8')])

    pinyin_bushou_vocab_ids = []
    for line in open(args.pinyin_bushou_ids):
        word_id, pinyin_ids, bushou_ids = line.rstrip('\r\n').split('\t')
        word_id = int(word_id)
        pinyin_ids = list(map(int, pinyin_ids.split()))
        bushou_ids = list(map(int, bushou_ids.split()))
        if not (word_id < word_vocab_size and max(pinyin_ids) < pinyin_vocab_size and max(bushou_ids) < bushou_vocab_size):
            continue
        pinyin_bushou_vocab_ids.append((word_id, pinyin_ids, bushou_ids))

    return pinyin_vocab_size, bushou_vocab_size, pinyin_bushou_vocab_ids
...
def main():
    ...
    parser.add_argument("--tgt_embedding", choices=["word", "word_pos", "word_pos_seg", "word_sinusoidalpos", "word_pos_seg_pinyin_bushou"],
                        default="word_pos_seg_pinyin_bushou", help="Target embedding type.")
    ...
    parser.add_argument("--pinyin_vocab", default=None, type=str,
                        help="Path of the pinyin vocab.")
    parser.add_argument("--bushou_vocab", default=None, type=str,
                        help="Path of the bushou vocab.")
    parser.add_argument("--pinyin_bushou_ids", default=None, type=str,
                        help="Path of the pinyin bushou ids.")
    ...
    pinyin_vocab_size, bushou_vocab_size, pinyin_bushou_vocab_ids = load_pinyin_bushou_vocab(args)

    # Build classification model.
    model = CText2text(args, pinyin_vocab_size, bushou_vocab_size, pinyin_bushou_vocab_ids)
