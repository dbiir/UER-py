...
def read_dataset(args, data_path, label_path):

    examples = read_chid_examples(data_path, label_path, is_training=True)
    features = convert_examples_to_features(examples, args.tokenizer , args.seq_length, args.max_choices_num)

    dataset = []
    for f in features:
        dataset.append((f.input_ids, f.label, f.segment_ids))
