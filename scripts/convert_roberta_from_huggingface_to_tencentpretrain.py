...
def convert_bert_transformer_encoder_from_huggingface_to_tencentpretrain(
    input_model, output_model, layers_num
):
...
def main():
    ...
    parser.add_argument(
        "--input_model_path", type=str, default="pytorch_model.bin", help="."
    )
    parser.add_argument(
        "--output_model_path", type=str, default="google_model.bin", help="."
    )
    ...
    output_model["embedding.word_embedding.weight"] = input_model[
        "roberta.embeddings.word_embeddings.weight"
    ]
    output_model["embedding.position_embedding.weight"] = input_model[
        "roberta.embeddings.position_embeddings.weight"
    ]
    output_model["embedding.segment_embedding.weight"] = torch.cat(
        (
            torch.Tensor(
                [
                    [0]
            ...
            input_model["roberta.embeddings.token_type_embeddings.weight"],
        ),
        dim=0,
    )
    output_model["embedding.layer_norm.gamma"] = input_model[
        "roberta.embeddings.LayerNorm.weight"
    ]
    output_model["embedding.layer_norm.beta"] = input_model[
        "roberta.embeddings.LayerNorm.bias"
    ]

    convert_bert_transformer_encoder_from_huggingface_to_tencentpretrain(
        input_model, output_model, args.layers_num
    )

    if args.target == "bert":
        output_model["target.nsp_linear_1.weight"] = input_model[
            "roberta.pooler.dense.weight"
        ]
        output_model["target.nsp_linear_1.bias"] = input_model[
            "roberta.pooler.dense.bias"
        ]
        output_model["target.nsp_linear_2.weight"] = input_model[
            "cls.seq_relationship.weight"
        ]
        output_model["target.nsp_linear_2.bias"] = input_model[
